...
Powered by Groq's lightning-fast API (using `openai/gpt-oss-20b`), SmartShell detects your operating system (Windows or Linux) and writes the correct syntax (PowerShell or Bash) automatically.
...
* **Failsafe Execution:** Built-in safety checks ensure the tool only runs when properly configured.
...
*(Note: If `pip` is not recognized, use `py -m pip install git+[https://github.com/avikumarzs/SmartShell.git](https://github.com/avikumarzs/SmartShell.git)`)*
...
If you need to change your API key or wipe your local data, use the reset command. This will delete your API key and history, locking the tool until you run `config` again.
`smart reset`
...
* **CLI Framework:** Rich (for terminal formatting) & Typer
